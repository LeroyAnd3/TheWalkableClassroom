...
function prepareCards(n) {
  ...
  for (i = 0; i < n; i++) {
    if (!paired) {
      ...
      while (addedCardList.includes(itemID)) {
        itemID = getRandomIntInclusive(0, terms.length - 1);
      }
    ...
    if (paired) {
      ...
      while (a == b) {
        b = getRandomIntInclusive(2, terms[itemID].length - 1);
      } //prevent a==b (i.e. same hint for same termID)
...
function hintFunction() {
  ...
  if (isAnimating) {
    console.log("Animation in progress, try hint functionality later");
    return;
  }
  var blankdiv = '<div class ="card" id="blank" style ="visibility:hidden" >' + " " + '</div>';
  ...
  if (clearedCardList.length == num) {
    console.log("No more hints to clear...");
    return;
  }
  while (clearedCardList.includes(i.toString())) {
    i = getRandomIntInclusive(0, num - 1);
  }
  isAnimating = true;
  if (i % 2 == 0) {
    ...
    clearedCardList.push(i.toString());
    clearedCardList.push((i + 1).toString());
  } else {
    //Highlight the cards about to be cleared
    setCardToYellow($('#card' + i));
    setCardToYellow($('#card' + (i - 1)));
...
function checkCardBackgrounds() {
  ...
  $('#naturalwhite').each(function() {
    if (this.selected) {
      $('.card').css("background-image", "url(./resources/ep_naturalwhite.png)");
    }
  });
  ...
  $('#sayagata').each(function() {
    if (this.selected) {
      $('.card').css("background-image", "url(./resources/sayagata-400px.png)");
    }
  });
  $('#retinawood').each(function() {
    if (this.selected) {
      $('.card').css("background-image", "url(./resources/retina_wood.png");
    }
  });
  $('#circlesround').each(function() {
    if (this.selected) {
      $('.card').css("background-image", "url(./resources/circles-and-roundabouts.png)");
    }
  });
  $('#oldmap').each(function() {
    if (this.selected) {
      $('.card').css("background-image", "url(./resources/old_map.png)");
    }
  });
  $('#skulls').each(function() {
    if (this.selected) {
      $('.card').css("background-image", "url(./resources/skulls.png)");
    }
  });
}
...
function setCardBackground(id) {
  ...
  $('#naturalwhite').each(function() {
    if (this.selected) {
      $('#' + id).css("background-image", "url(./resources/ep_naturalwhite.png)");
    }
  });
  $('#naturalblack').each(function() {
    if (this.selected) {
      $('#' + id).css("background-image", "url(./resources/ep_naturalblack.png)");
    }
  });
  $('#sayagata').each(function() {
    if (this.selected) {
      $('#' + id).css("background-image", "url(./resources/sayagata-400px.png)");
    }
  });
  $('#retinawood').each(function() {
    if (this.selected) {
      $('#' + id).css("background-image", "url(./resources/retina_wood.png");
    }
  });
  $('#circlesround').each(function() {
    if (this.selected) {
      $('#' + id).css("background-image", "url(./resources/circles-and-roundabouts.png)");
    }
  });
  $('#oldmap').each(function() {
    if (this.selected) {
      $('#' + id).css("background-image", "url(./resources/old_map.png)");
    }
  });
  $('#skulls').each(function() {
    if (this.selected) {
      $('#' + id).css("background-image", "url(./resources/skulls.png)");
    }
  });
}
